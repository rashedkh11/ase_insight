...
def validate_data(df, date_column, price_column):
    ...
    if missing_dates > 0:
        issues.append(f"⚠️ {missing_dates} missing values in date column")
    if missing_prices > 0:
        issues.append(f"⚠️ {missing_prices} missing values in price column")
    ...
    if outliers > 0:
        issues.append(f"⚠️ {outliers} potential outliers detected")
    ...
    if validation_issues:
        with st.expander("⚠️ Data Quality Issues", expanded=True):
            ...
            if st.button("Clean Data"):
                ...
                # إعادة حساب المشاكل بعد التنظيف
                validation_issues = validate_data(df, date_column, price_column)
            ...
            with st.spinner("Preparing data..."):
                ...
                if model_category == "Deep Learning (LSTM/GRU/RNN)":
                    ...
                    if model_mae < naive_mae:
                        ...
                        st.warning("⚠️ Model doesn't beat naive - possible issue")
                    ...
                    if avg_error_pct < 1.0:
                        ...
                        st.info(f"ℹ️ Average error: {avg_error_pct:.2f}% - Good")
                    else:
                        st.warning(f"⚠️ Average error: {avg_error_pct:.2f}% - Needs improvement")
